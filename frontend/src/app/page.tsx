...
const Carousel3D = dynamic(() => import('../components/three/Carousel3D'), { ssr: false })

export default function Home() {
  const [isLoaded, setIsLoaded] = useState(false)
  const [themeMode, setThemeMode] = useState<'classic' | 'carousel'>('classic')
...
<body>
  ...
  useEffect(() => {
    setIsLoaded(true)
    try {
      const saved = typeof window !== 'undefined' ? localStorage.getItem('page_theme_mode') : null
      if (saved === 'carousel' || saved === 'classic') setThemeMode(saved)
    } catch { }
  }, [])

  const toggleThemeMode = () => {
    setThemeMode(prev => {
      const next = prev === 'classic' ? 'carousel' : 'classic'
      try { localStorage.setItem('page_theme_mode', next) } catch { }
      return next
    })
  }

  return (
    <div className={`${themeMode === 'carousel' ? 'min-h-screen flex flex-col bg-black' : 'min-h-screen bg-gradient-to-br from-slate-900 via-teal-900 to-emerald-900 overflow-hidden'}`}>
      {/* Animated Background Elements (apenas no modo clássico) */}
      {themeMode === 'classic' && (
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            <div className="absolute right-4 -top-6">
              <button
                ...
                className="flex items-center gap-2 px-3 py-1.5 rounded-full border border-white/15 bg-white/5 text-white/70 hover:text-white hover:bg-white/10 transition-colors text-xs"
                aria-label="Alternar tema da página"
              >
                <span className={`inline-block w-2 h-2 rounded-full ${themeMode === 'carousel' ? 'bg-cyan-400' : 'bg-emerald-400'}`}></span>
                {themeMode === 'carousel' ? 'Carrossel' : 'Clássico'}
              </button>
            </div>
            <div className="text-center">
      ...
      {themeMode === 'carousel' && (
        <div className="fixed bottom-4 right-4 z-50">
          <button
            onClick={toggleThemeMode}
            className="flex items-center gap-2 px-3 py-1.5 rounded-full border border-white/15 bg-white/10 backdrop-blur text-white/80 hover:text-white hover:bg-white/20 transition-colors text-xs"
            aria-label="Alternar tema da página"
          >
            <span className={`inline-block w-2 h-2 rounded-full ${themeMode === 'carousel' ? 'bg-cyan-400' : 'bg-emerald-400'}`}></span>
            {themeMode === 'carousel' ? 'Carrossel' : 'Clássico'}
